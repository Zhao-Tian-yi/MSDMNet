...
def get_metrics_reduced(img1, img2):
    ...
    m1 = psnr_loss(img1, img2, 255.)
    # m2 = ssim(img1, img2, 11, 'mean', 255.)
    m3 = scc(img1, img2)
    m4 = sam(img1, img2)
    m5 = ergas(img1, img2)
    img1 = np.array(img1[0].permute(1, 2, 0).cpu())
    img2 = np.array(img2[0].permute(1, 2, 0).cpu())
    m6 = Qindex(img1, img2)
    return [m1.item(), m3.item(), m4.item(), m5.item(), float(m6),]

def get_metrics_full(HRMS,LRMS,PAN):
    HRMS = np.array(HRMS[0].permute(1, 2, 0).cpu())
    LRMS = np.array(LRMS[0].permute(1, 2, 0).cpu())
    PAN = np.array(PAN[0].permute(1, 2, 0).cpu())

    m1=qnr(HRMS,LRMS,PAN)
    m2=D_s(HRMS,LRMS,PAN)
    m3=D_lambda(HRMS,LRMS)
...
def _ssim(img1, img2, dynamic_range=255):
    """SSIM for 2D (one-band) image, shape (H, W); uint8 if 255; uint16 if 2047"""
...
def D_s(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=16, q=1):
    """Spatial distortion
    img_fake, generated HRMS
    img_lm, LRMS
    pan, HRPan"""
